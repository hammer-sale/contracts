...
contract OperatorBondTest is HammerBase {
    ...
    /// a slash distributes the WHOLE pool: victims pro-rata by recorded harm, remainder to Treasury,
    ...
    function test_SlashDistributesProRataAndConserves() public {
        ...
        assertEq(operatorBond.bondOf(sid), 10 ether, "pool = sum of stakes");
        ...
        assertEq(address(treasury).balance - treasuryBefore, 6 ether, "remainder to Treasury");

        uint256 v1 = bidder1.balance;
        vm.prank(bidder1);
        operatorBond.claimSlash(sid);
        assertEq(bidder1.balance - v1, 3 ether, "victim1 pro-rata (4 * 3/4)");

        uint256 v2 = bidder2.balance;
        vm.prank(bidder2);
        operatorBond.claimSlash(sid);
        assertEq(bidder2.balance - v2, 1 ether, "victim2 pro-rata (4 * 1/4)");
        ...
        assertEq(address(operatorBond).balance, 0, "bond pool fully distributed (conserved)");
    }

    /// a clean (no-claim) session lets each operator reclaim its full stake.
    function test_CleanSessionWithdrawReturnsFullStake() public {
        ...
        vm.prank(operator1);
        operatorBond.withdraw(sid);
        assertEq(operator1.balance - before, 5 ether, "clean withdraw returns the full stake");
        assertEq(operatorBond.bondOf(sid), 0, "pool drained on withdraw");
    }

    /// an open (recorded-but-unsettled) claim gates withdraw with WindowOpen.
    ...
    /// a registered clone cannot inflate a DIFFERENT session's claim ledger; recordClaim is bound to
    ...
    /// an unregistered EOA cannot record a claim (onlyAuction).
    ...
    /// a non-liveness slash with no victim claims routes the whole pool to Treasury.
    function test_NonLivenessSlashRoutesWholePoolToTreasury() public {
        ...
        assertEq(address(treasury).balance - treasuryBefore, 3 ether, "whole pool to Treasury (no victims)");
    ...
    /// withdraw before the session closes reverts SessionNotClosed. The close gate stops an operator
    ...
    /// a session's slash + victim claims never draw down another session's stake from the shared
    ...
    function test_CrossSessionPoolIsolationOnSlash() public {
        ...
        assertEq(address(operatorBond).balance, 15 ether, "singleton holds both sessions' stake");
        ...
        assertEq(address(operatorBond).balance, 5 ether, "session B stake intact after A fully slashed");
        assertEq(operatorBond.bondOf(sidB), 5 ether, "session B pool intact");
        ...
        assertEq(operator2.balance - before, 5 ether, "session B operator reclaims full stake");
        assertEq(address(operatorBond).balance, 0, "singleton conserved across both sessions");
    }

    /// closeSession is permissionless but reverts SessionNotClosed before the session's verifiable
    ...
    /// a registered clone can only close the session it owns. Closing a foreign sessionId through a
    ...
    /// once a session is closed the claim ledger is sealed; a late recordClaim reverts
    ...
    /// the ERC-20 bond rail. An operator stakes the session's ERC-20; a slash distributes the TOKEN
    ...
    function test_Erc20BondSlashDistributesAndConserves() public {
        ...
        assertEq(operatorBond.bondOf(sidE), 10e6, "token stake pooled");
        assertEq(operatorBond.sessionToken(sidE), address(token), "session rail is the ERC-20");
        assertEq(token.balanceOf(address(operatorBond)), 10e6, "token held by the bond");
        ...
        assertEq(token.balanceOf(address(treasury)) - treasuryBefore, 7e6, "token remainder to Treasury");

        uint256 victimBefore = token.balanceOf(bidder1);
        vm.prank(bidder1);
        operatorBond.claimSlash(sidE);
        assertEq(token.balanceOf(bidder1) - victimBefore, 3e6, "victim paid token harm pro-rata");
        assertEq(token.balanceOf(address(operatorBond)), 0, "token bond fully distributed (conserved)");
    }

    /// a clean ERC-20 session returns the operator's full TOKEN stake on close + withdraw.
    function test_Erc20CleanWithdrawReturnsFullTokenStake() public {
        ...
        assertEq(token.balanceOf(operator1) - before, 7e6, "clean ERC-20 withdraw returns full token stake");
        assertEq(operatorBond.bondOf(sidE), 0, "token pool drained on withdraw");
    }

    /// cross-denomination isolation on the shared singleton. A NATIVE session fully slashed does not
    ...
    function test_CrossDenominationIsolation() public {
        ...
        assertEq(address(operatorBond).balance, 10 ether, "native balance = native stake only");
        assertEq(token.balanceOf(address(operatorBond)), 8e6, "token balance = token stake only");
        ...
        assertEq(token.balanceOf(address(operatorBond)), 8e6, "ERC-20 pool intact after native slash");
        assertEq(operatorBond.bondOf(sidE), 8e6, "ERC-20 session pool intact");
        assertEq(address(operatorBond).balance, 0, "native rail fully distributed");
        ...
        vm.prank(operator2);
        operatorBond.withdraw(sidE);
        assertEq(token.balanceOf(operator2) - before, 8e6, "ERC-20 operator reclaims full token stake");
    }

    /// a fee-on-transfer token credits the measured received amount, not the requested amount, so the
    ...
    function test_Erc20FeeOnTransferCreditsReceivedNotRequested() public {
        ...
        assertEq(operatorBond.bondOf(sidF), received, "bond credits the MEASURED received amount, not requested");
        assertEq(feeToken.balanceOf(address(operatorBond)), received, "pool == actual token balance held");
    }

    /// a victim harmed on multiple distinct seqs is paid the sum of its recorded harms, not the
    ///       single largest.
